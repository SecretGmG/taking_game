...
impl DenseTakingGame {
    ...
    fn is_valid_match(
        ...
        // Nodes must not share a hyperedge
        if (neighbourhoods[node] & (1 << candidate)) != 0 {
            return false;
        }

        let candidate_neighbours = neighbourhoods[candidate];

        // All already-mapped neighbours of `node` must map into candidate's neighbours
        for (neighbour_idx, mapped) in symmetries.iter().enumerate() {
            if let Some(mapped_node) = mapped {
                if (neighbourhoods[node] & (1 << neighbour_idx)) != 0 {
                    if (candidate_neighbours & (1 << mapped_node)) == 0 {
                        return false;
                    }
                }
            }
    ...
    fn get_neighbourhoods(&self) -> Vec<u128> {
        let mut neighbourhoods: Vec<u128> = vec![0; self.nodes.len()];
        let dual = self.hypergraph_dual();
        for node in 0..self.nodes.len() {
            for &e in &dual[node] {
                neighbourhoods[node] |= self.hyperedges[e];
            }
        }
        neighbourhoods
    }
