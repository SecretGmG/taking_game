...
pub struct DenseConstructor {
    hyperedges: Vec<Vec<usize>>,
}
impl DenseConstructor {
    ...
    pub fn from_hyperedges(hyperedges: Vec<Vec<usize>>) -> DenseConstructor {
        DenseConstructor { hyperedges }
    }
    /// Returns a graph with one empty set (no nodes).
    pub fn empty() -> DenseConstructor {
        DenseConstructor::from_hyperedges(vec![vec![]])
    }
    /// Returns a graph with one set containing a single node.
    pub fn unit() -> DenseConstructor {
        DenseConstructor::from_hyperedges(vec![vec![0]])
    }
    pub fn heap(size: usize) -> DenseConstructor {
        DenseConstructor::from_hyperedges(vec![(0..size).collect()])
    }
    /// Constructs a Kayles game of the given size.
    ///
    /// Each set connects two adjacent nodes. Returns `empty()` if size == 0,
    /// and `unit()` if size == 1.
    pub fn kayles(size: usize) -> DenseConstructor {
        if size == 0 {
            return DenseConstructor::empty();
        }
        if size == 1 {
            return DenseConstructor::unit();
        }
        let mut hyperedges = vec![];
        for i in 1..size {
            hyperedges.push(vec![i - 1, i]);
        }
        DenseConstructor::from_hyperedges(hyperedges)
    }
    ...
    pub fn triangle(l: usize) -> DenseConstructor {
        let mut hyperedges = vec![];
        for i in 0..l {
            let mut h1 = Vec::new();
            let mut h2 = Vec::new();
    ...
    pub fn rect(x: usize, y: usize) -> DenseConstructor {
    ...
    pub fn hyper_cube(dim: usize, l: usize) -> DenseConstructor {
    ...
    pub fn hyper_cuboid(lengths: Vec<usize>) -> DenseConstructor {
    ...
    pub fn hyper_tetrahedron(dim: usize) -> DenseConstructor {
        let mut g = Self::unit();
        for _ in 0..dim {
    ...
    pub fn build(self) -> Vec<DenseTakingGame> {
        DenseTakingGame::from_hyperedges(self.hyperedges)
    }
    pub fn build_one(self) -> DenseTakingGame {
    ...
    pub fn connect_unit_to_all(self) -> DenseConstructor {
    ...
    pub fn fully_connect(mut self, other: &Self) -> DenseConstructor {
    ...
    pub fn extrude(mut self, l: usize) -> DenseConstructor {
